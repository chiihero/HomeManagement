    ...
    <el-card class="header-card mb-6 border-0 shadow-sm">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex items-center gap-2">
          <el-button type="primary" @click="openAddEntityForm" class="flex items-center gap-2">
            <el-icon><Plus /></el-icon>添加物品
          </el-button>
          <el-button :icon="Refresh" circle @click="loadTreeData" :loading="loading" />
        </div>
      </div>
    </el-card>
      ...
      <el-card class="detail-container grow border-0 shadow-sm">
        <template #header>
          <div class="flex items-center justify-between">
            ...
            <el-tooltip effect="dark" content="刷新" placement="top" v-if="currentEntity && !isEditing && !isAdding">
              <el-button :icon="Refresh" circle plain size="small" @click="refreshCurrentEntity" />
            </el-tooltip>
          </div>
        </template>
...
const filteredTreeData = computed(() => {
  ...
  const filterTree = (nodes: Entity[]) => {
    return nodes.filter(node => {
      // 检查当前节点是否匹配
      const isMatch = node.name.toLowerCase().includes(searchKeyword.value.toLowerCase())
...
const getDetailTitle = computed(() => {
  ...
  return currentEntity.value ? `物品详情: ${currentEntity.value.name}` : '物品详情'
})
...
.tree-wrapper, .detail-container {
  background-color: white;
  transition: all 0.3s ease;
}

.tree-wrapper:hover, .detail-container:hover {
