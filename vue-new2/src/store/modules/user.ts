...
export const useUserStore = defineStore("pure-user",{
  state: (): UserState => ({
    ...
    tokenExpiry: localStorage.getItem('token_expiry') ? 
      parseInt(localStorage.getItem('token_expiry') || '0') : null,
    user: null,
    loading: false,
  ...
  actions: {
    ...
    /** 登录 */
    async loginByUsername(data: { username: string; password: string; remember?: boolean }) {
      this.loading = true;
      try {
        const response = await login(data.username, data.password, data.remember || false);
