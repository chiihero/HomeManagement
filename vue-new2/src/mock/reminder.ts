...
const generateReminders = (count: number) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const notificationMethods = methods.slice(0, Math.floor(Math.random() * methods.length) + 1)
    const isRecurring = Math.random() > 0.5
    const recurringCycle = isRecurring ? cycles[Math.floor(Math.random() * cycles.length)] : undefined
...
export default [
  {
    ...
    response: ({ query }) => {
      ...
      if (type) {
        filteredReminders = filteredReminders.filter(item => item.type === type)
      }
      if (status) {
        filteredReminders = filteredReminders.filter(item => item.status === status)
      }
      if (dateRange) {
